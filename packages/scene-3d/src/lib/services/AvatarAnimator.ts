 ...
 * Coordinates between hand targets and skeleton IK solving.
 *
 * Adapted from TKA's AvatarAnimator: TKA-specific PropState3D methods
 * (setHandTargetsFromProps, setPropsAndBlend) are removed. Consumers
 * set hand targets directly via setLeftHandTarget/setRightHandTarget
 * using the generic HandPose interface.
 */
...
import type {
  ...
  AnimationLayer,
  TransitionConfig,
} from './contracts/IAvatarAnimator.js';
import type { IIKSolver, IKTarget } from './contracts/IIKSolver.js';
import type {
  IAvatarSkeletonBuilder,
  BoneName,
} from './contracts/IAvatarSkeletonBuilder.js';
import type { IElbowPoleComputer } from './contracts/IElbowPoleComputer.js';
import type { IClavicleRaiser } from './contracts/IClavicleRaiser.js';
import type { ISpineTwister } from './contracts/ISpineTwister.js';
...
export class AvatarAnimator implements IAvatarAnimator {
  ...
  private leftArmIK = { weight: 1, targetWeight: 1 };
  private rightArmIK = { weight: 1, targetWeight: 1 };
  ...
  constructor(
    ...
    // Default pose: no targets in either hand. Hands become non-null once
    // setLeftHandTarget / setRightHandTarget is called.
    this.currentPose = {
      leftHand: null,
      rightHand: null,
      timestamp: Date.now(),
  ...
  update(deltaTime: number): void {
    ...
    this.leftArmIK.weight +=
      (this.leftArmIK.targetWeight - this.leftArmIK.weight) * blendFactor;
    this.rightArmIK.weight +=
      (this.rightArmIK.targetWeight - this.rightArmIK.weight) * blendFactor;
  ...
  private blendToTarget(): void {
    // Hands snap directly to targets - no lerp.
    // This keeps the wrist bones strictly locked to the target location
  ...
  private updateTransition(deltaTime: number): void {
    ...
    // side we cannot lerp, so snap to the end's nullability.
  ...
  private lerpHand(
    ...
    if (end.wristRotation) result.wristRotation = end.wristRotation.clone();
    if (end.gripType) result.gripType = end.gripType;
    if (end.plane) result.plane = end.plane;
    return result;
  }
  ...
  private computeFinalPose(): BodyPose {
    ...
    // both the current pose and the layer's pose.
  ...
  private applyIKToSkeleton(pose: BodyPose): void {
    ...
    if (
      ...
      // Skip Hips counter-rotation when configured - prevents hip yaw from
      // cascading to planted feet and making them slide on the ground.
    ...
    if (
      Math.abs(this.externalSpinePitchRad) > 0.0001 &&
      this.spineRestCached
    ) {
    ...
    if (leftChain && leftHand) {
      if (this.leftArmIK.weight > 0.001) {
        ...
        leftChain.middle.quaternion
          .copy(animMiddleQuat)
          .slerp(ikMiddleQuat, w);
        leftChain.effector.quaternion
          .copy(animEffectorQuat)
          .slerp(ikEffectorQuat, w);
      }
    ...
    if (rightChain && rightHand) {
      if (this.rightArmIK.weight > 0.001) {
        ...
        rightChain.middle.quaternion
          .copy(animMiddleQuat)
          .slerp(ikMiddleQuat, w);
        rightChain.effector.quaternion
          .copy(animEffectorQuat)
          .slerp(ikEffectorQuat, w);
      }
      // else: weight ~0, skip IK entirely - animation drives the arm
