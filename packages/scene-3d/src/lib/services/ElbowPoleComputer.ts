...
export class ElbowPoleComputer implements IElbowPoleComputer {
  computePoleVector(
    ...
    switch (plane) {
      ...
      default:
        // Exhaustiveness safety net - all MotionPlane values are handled above
  ...
  private computeWallPole(
    ...
    const crossBody = Math.max(
      0,
      Math.min(1, (-localX * sideSign) / SHOULDER_HALF_WIDTH),
    );
  ...
  private computeFloorPole(
    ...
    const centerProximity = Math.max(
      0,
      Math.min(1, 1 - horizontalDist / SHOULDER_HALF_WIDTH),
    );
