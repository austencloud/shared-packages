 ...
 * - Thoracic spine: ~47 deg total axial rotation capacity
 * - Cervical spine: ~85 deg total axial rotation capacity
 * - Lateral flexion: ~25 deg thoracic, ~45 deg cervical
 * - We use ~60 deg max yaw, ~25 deg max tilt - within safe range
 */
...
import type {
  ISpineTwister,
  SpineTwistResult,
} from './contracts/ISpineTwister.js';
...
const HIP_COUNTER_FRACTION = -0.2;
...
export class SpineTwister implements ISpineTwister {
  ...
  private computeTwoHandedTwist(
    ...
    const normalizedYaw = Math.max(
      -1,
      Math.min(1, yawSignal / SHOULDER_HALF_WIDTH),
    );
    ...
    const heightFactor = Math.max(
      0,
      Math.min(1, (avgHeight - TILT_HEIGHT_THRESHOLD) / 0.3),
    );

    // Cross-body factor: 0 = hands on their natural sides, 1 = fully crossed
    const crossFactor = Math.max(
      0,
      Math.min(1, (leftCross + rightCross) / SHOULDER_HALF_WIDTH),
    );
    ...
    return {
      spine1: this.makeSpineRotation(
        totalYaw * (weights.spine1 ?? 0),
        totalTilt * (weights.spine1 ?? 0),
      ),
      spine2: this.makeSpineRotation(
        totalYaw * (weights.spine2 ?? 0),
        totalTilt * (weights.spine2 ?? 0),
      ),
      neck: this.makeSpineRotation(
        totalYaw * (weights.neck ?? 0),
        totalTilt * (weights.neck ?? 0),
      ),
      head: this.makeSpineRotation(
        totalYaw * (weights.head ?? 0),
        totalTilt * (weights.head ?? 0),
      ),
      hips: this.makeSpineRotation(
        totalYaw * HIP_COUNTER_FRACTION,
        totalTilt * HIP_COUNTER_FRACTION * 0.5,
      ),
    };
  }
  ...
  private computeSingleHandGaze(
    ...
    const normalizedYaw = Math.max(
      -1,
      Math.min(1, offsetX / SINGLE_HAND_REACH),
    );
