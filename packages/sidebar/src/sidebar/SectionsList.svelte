...
<!-- Container for section buttons that can slide in/out.
     ...
     group headers. Otherwise renders the flat list, unchanged. -->
<script lang="ts">
  import { slide, fly } from 'svelte/transition';
  import { cubicOut } from 'svelte/easing';
  import type { Snippet } from 'svelte';
  import type { Section, SectionGroup } from '../types';
  ...
  // section is tagged — AND we're expanded. In the rail the tabs flatten to a
  // plain icon list (group headers carry labels that don't fit the 64px rail).
  ...
  const storeKey = $derived(`nav-expanded-${moduleId}`);
  ...
  let expanded = $state<Record<string, boolean>>(loadExpanded(`nav-expanded-${moduleId}`));
      ...
      >
        ...
        {#if group.icon}
          <span class="group-icon">
            {#if renderIcon}{@render renderIcon(group.icon, 18)}{:else}{@html group.icon}{/if}
          </span>
        {/if}
...
<style>
  ...
  .sections-list {
    ...
    /* The rail<->expanded inset lives here — the single horizontal motion of the
       whole tab list. Expanded insets tabs 10px right of the module icon; rail
       zeroes it so tab icons sit directly under the module (x=32). Transitioning
       padding-left slides every tab together, no snap. */
    padding-left: 10px;
