 ...
 *   --user <admin|email>            User profile (default: admin)
 *
 * Examples:
...
import config from "@austencloud/feedback-services/config";

// Project root is current working directory
const PROJECT_ROOT = process.cwd();

// Known user profiles - use config for admin profile
const USER_PROFILES = {
  admin: {
  ...
  claude: {
    userId: "system",
    userEmail: process.env.CLAUDE_EMAIL || "claude@example.com",
...
// Service account in project root
...
function parseArgs() {
  ...
  if (args.length < 2) {
    ...
    console.error(
      "  --user <admin|claude|email>              User profile (default: admin)"
    );
    console.error("");
  ...
  const options = {
    ...
    tab: "general",
    subtasks: null,
    user: "admin", // Default to Austen's profile
  };
...
async function submitFeedback(params) {
  try {
    ...
    if (USER_PROFILES[params.user]) {
      ...
      console.error(
        `❌ Unknown user "${params.user}". Use: admin, claude, or an email address.`
      );
      process.exit(1);
    }
    ...
    console.log("");
    console.log(`   🔗 View in Kanban: Open Ringmaster → Feedback Manage tab`);
