<script lang="ts">
  ...
  const activeConfig = $derived(config ?? createDefaultWinterConfig());
  ...
  const sceneRef = (scene as any).current ?? scene;
  ...
  $effect(() => {
    ...
    sceneRef.fog = new FogExp2(new Color(fog.color), fog.density);
    return () => {
      if (sceneRef) sceneRef.fog = null;
    };
  });
