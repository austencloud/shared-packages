/**
 * Cosmic Service Factory
 *
 * Creates and caches all Cosmic-specific services including
...
import { CosmicCalculationService } from "./implementations/CosmicCalculationService.js";
...
/**
 * Container for all Cosmic services
 */
export interface CosmicServices {
  calculationService: ICosmicCalculationService;
...
// Cached services
let cachedServices: CosmicServices | null = null;

/**
 * Get or create all Cosmic services
 */
export function getCosmicServices(): CosmicServices {
  if (!cachedServices) {
    // Create calculation service first (needed by other services)
    const calculationService = new CosmicCalculationService();
 ...
 */
export function resetCosmicServices(): void {
  cachedServices = null;
}
