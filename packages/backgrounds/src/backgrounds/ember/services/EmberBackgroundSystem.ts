...
import type { EmberLayers, EmberStats } from "../domain/models/ember-models.js";
import {
  EMBER_BACKGROUND_GRADIENT,
  EMBER_QUALITY_CONFIGS,
  DENSITY_MULTIPLIERS,
 ...
 */
export class EmberBackgroundSystem implements IBackgroundSystem {
  // State
  ...
  public initialize(dimensions: Dimensions, quality: QualityLevel): void {
    ...
    const config = EMBER_QUALITY_CONFIGS[quality];
   ...
   */
  public setLayerVisibility(layers: Partial<EmberLayers>): void {
  ...
  public setHeatIntensity(intensity: HeatIntensity): void {
    ...
    if (this.isInitialized) {
      const config = EMBER_QUALITY_CONFIGS[this.quality];
      const densityMult = DENSITY_MULTIPLIERS[this.densityPreset];
