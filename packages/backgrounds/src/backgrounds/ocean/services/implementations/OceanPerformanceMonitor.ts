...
export interface IOceanPerformanceMonitor {
  startFrame(): void;
  endUpdate(): void;
  endRender(
    fishCount: number,
...
/**
 * Lightweight performance monitor for ocean background.
 * Tracks FPS, frame times, and entity counts.
 * Disabled by default - enable via console or dev tools.
 */
export class OceanPerformanceMonitor implements IOceanPerformanceMonitor {
...
let _monitorInstance: OceanPerformanceMonitor | null = null;

export function setMonitorInstance(monitor: OceanPerformanceMonitor): void {
  _monitorInstance = monitor;

  // Expose to console for debugging
  if (typeof window !== "undefined") {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (window as any).enableOceanPerf = () => {
      monitor.setEnabled(true);
    };
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (window as any).disableOceanPerf = () => {
      monitor.setEnabled(false);
    };
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (window as any).getOceanMetrics = () => {
