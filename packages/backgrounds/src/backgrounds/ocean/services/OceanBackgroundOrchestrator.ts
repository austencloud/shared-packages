...
import type { OceanState, FishMarineLife, JellyfishMarineLife } from "../domain/models/OceanModels.js";
...
import {
  OceanPerformanceMonitor,
  setMonitorInstance,
} from "./implementations/OceanPerformanceMonitor.js";

// Factory for service creation
import { getOceanServices } from "./OceanServiceFactory.js";

/**
 * Ocean Background Orchestrator
 *
...
export class OceanBackgroundOrchestrator implements IBackgroundSystem {
  ...
  constructor(
    ...
    // Expose perf monitor to console (call enableOceanPerf() in console to activate)
   ...
   */
  static create(): OceanBackgroundOrchestrator {
    const services = getOceanServices();
  ...
  private createEmptyState(): OceanState {
    return {
      bubbles: [],
      fish: [],
      jellyfish: [],
  ...
  setLayerVisibility(layers: Partial<OceanLayers>): void {
    this.layerVisibility = { ...this.layerVisibility, ...layers };
  }

  getLayerVisibility(): OceanLayers {
